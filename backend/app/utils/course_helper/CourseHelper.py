...
import pandas as pd
from utils.course_helper.encrypt import encrypt
...
class CourseHelper:
    ...
    -----END PUBLIC KEY-----'''
        ...
        # 获取选课信息url
        self.course_rank_url = 'http://xk.autoisp.shu.edu.cn/StudentQuery/QueryEnrollRankList'
    ...
    def login(self):
        ...
        logger.info('尝试登录...用户名: {}, 密码: {}, 加密后的密码: {}', self.stu_id,
                    self.password, encrypt(self.password, self.pubkey))
        try:
    ...
    def select_term(self):
        ...
        # 解析 termid
        term_index_response = self.session.get(
            self.term_index_url, headers=self.headers)
    ...
    def get_course_rank(self, save_type='') -> pd.DataFrame:
        """
        获取选课排名
        :param save_type: 保存格式, 若空则不保存
        :return: 选课排名表
        """
        response = self.session.get(self.course_rank_url, headers=self.headers)
        selector = etree.HTML(response.text)
        course_elements = selector.xpath('//tr[@name="rowclass"]')
        course_list = []
        for course_element in course_elements:
            course_list.append([field.strip()
                                for field in course_element.xpath('./td/text()')])
        # switch to DataFrame
        columns = ['course_id', 'course_name', 'teacher_id',
                   'teacher_name', 'capacity', 'number', 'rank']
        df = pd.DataFrame(course_list, columns=columns)
        if save_type:
            logger.info('保存选课排名')
            self.save_to_path(df, name=self.term_season +
                              '季学期 选课排名', type=save_type)
        return df
    ...
    def get_stu_info(self, save_type='') -> pd.DataFrame:
        ...
        response = self.session.get(
            self.course_table_url, headers=self.headers)
        selector = etree.HTML(response.text)
        # 获取学生信息
        stu_info_list = [field.strip().strip('：') for field in selector.xpath(
            '//td[@style="color: Blue;"]/text()') if field.strip().strip('：')]
        stu_credit_raw = selector.xpath(
            '//table[@class="tblnoborder"]//tr/td[@colspan="20"]/text()')[0]  # 获取学分
        stu_info_list.append(
            re.search('Total credit：(\d+\.\d+)', stu_credit_raw).group(1))

        # switch to DataFrame
        stu_info_columns = ['stu_id', 'name', 'sexual',
                            'grade', 'academy', 'campus', 'credit']
        try:
            stu_info_df = pd.DataFrame(
                [stu_info_list], columns=stu_info_columns)
            if save_type:
                logger.info('保存个人信息')
                self.save_to_path(
                    stu_info_df, name=self.term_season + '季学期 个人信息', type=save_type)
            return stu_info_df
        except:
    ...
    def get_course_info(self, save_type='') -> pd.DataFrame:
        ...
        for one_piece in course_info_element.xpath('.//tr[@name="rowclass"]'):
            # 记录一门课程的字段信息
            course_info_list.append([field.xpath('./text()')[0].strip() if field.xpath(
                './text()') else '' for field in one_piece.xpath('./td')])
            # one_piece_list = []
        ...
        # switch to DataFrame
        course_info_columns = [
            'id', 'course_id', 'course_name', 'credit',
            'teacher_id', 'teacher_name', 'course_time', 'course_location',
            'question_time', 'question_location', 'campus']
        course_info_df = pd.DataFrame(
            course_info_list, columns=course_info_columns)
        try:
            course_info_df = pd.DataFrame(
                course_info_list, columns=course_info_columns)
            if save_type:
                logger.info('保存课程信息')
                self.save_to_path(
                    course_info_df, name=self.term_season + '季学期 课程信息', type=save_type)
    ...
    def get_course_table(self, save_type='') -> pd.DataFrame:
        ...
        for one_piece in course_table_element.xpath('.//tr[@name="rowweek"]'):
            ...
            course_table_list.append([field.strip().replace('\n', '').replace(
                ' ', '') for field in one_piece.xpath('./td/text()')])
        # switch to DataFrame
        course_table_columns = ['id', 'time', 'Mon',
                                'Tues', 'Wed', 'Thur', 'Fri', 'Sat', 'Sun']

        try:
            course_table_df = pd.DataFrame(
                course_table_list, columns=course_table_columns)
            if save_type:
                logger.info('保存课程表')
                self.save_to_path(
                    course_table_df, name=self.term_season + '季学期 课程表', type=save_type)
    ...
    def choose_course(self, course_info: list) -> list:
        ...
        response = self.session.post(
            self.course_selection_url, data=post_data, headers=self.headers)
        ...
        for course in selection_feedback_element:
            feedback_course_info.append(
                [field.strip() for field in course.xpath('.//td/text()')])
        for feedback_course in feedback_course_info:
            logger.info(feedback_course[1]+' '+feedback_course[3]+' '+feedback_course[4] +
                        ' '+feedback_course[2]+' '+feedback_course[-1])  # TODO use logger
    ...
    def is_full(self, course_id: str, teacher_id: str) -> bool:
        ...
        response = self.session.post(
            self.course_search_url, data=post_data, headers=self.headers)
